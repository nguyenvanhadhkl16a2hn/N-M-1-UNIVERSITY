...
class Dot():
    ...
    def update(self):
        if self.size >0:
            self.size = self.size - SPEED_CHANGE_SIZE * 5
        else:
            self.size = 0
    def draw(self):
        if self.size >0:
...
class Random():
    ...
    def randomBulletFlyUp_speed():
        speed = random.randint(SPEED_FLY_UP_MIN,SPEED_FLY_UP_MAX)
        return speed
...
def main():
    ...
    pygame.init()
    pygame.display.init()
    pygame.display.set_caption("BY OOTSUTSUKI MURA")
    ...
    while True:
        ...
        if time == TIME_CREAT_FW:
            for i in range(Random.num_fireworks()):
                bulletFlyUps.append(BulleFlyUp(Random.randomBulletFlyUp_speed(),Random.randomBulletFlyUp_x()))
